import { TransactionState } from '../orchestrator/transaction'
import { LocalWorkflow } from '../orchestrator/workflow/local-workflow'
import { WorkflowScheduler } from '../orchestrator/workflow/scheduler'
import { WorkflowManager } from '../orchestrator/workflow/workflow-manager'
...
describe('WorkflowManager', () => {
  ...
  it('should begin a transaction and returns its final state', async () => {
    const flow = new LocalWorkflow('create-product')
  ...
  it('should begin a transaction and revert it when fail', async () => {
    const flow = new LocalWorkflow('broken-delivery')
    const transaction = await flow.run('t-id')
  ...
  it('should continue an asyncronous transaction after reporting a successful step', async () => {
    const flow = new LocalWorkflow('deliver-product')
    const transaction = await flow.run('t-id')
  ...
  it('should update a flow with a new step and a new handler', async () => {
    ...
    expect(WorkflowManager.getWorkflow('create-product')?.handlers_.has('xor')).toEqual(false)
  })
