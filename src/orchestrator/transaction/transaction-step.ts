...
export class TransactionStep {
  ...
  canRetryAwaiting(): boolean {
    return !!(
      ...
      this.lastAttempt &&
      Date.now() - this.lastAttempt > this.definition.retryIntervalAwaiting! * 1e3
    )
  }
  ...
  canCancel(): boolean {
    return (
      !this.isCompensating() &&
      [TransactionStepStatus.WAITING, TransactionStepStatus.TEMPORARY_FAILURE].includes(
        this.getStates().status,
      )
    )
  }
}
