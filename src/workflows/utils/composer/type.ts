...
export type StepFunctionReturnConfig<TOutput> = {
  config(
    config: { name?: string } & Omit<TransactionStepsDefinition, 'next' | 'uuid' | 'action'>,
  ): WorkflowData<TOutput>
}
...
export type ReturnWorkflow<TData, TResult, THooks extends any[]> = {
  ...
  run: <TDataOverride = undefined, TResultOverride = undefined>(
    ...args: Parameters<ExportedWorkflow<TData, TResult, TDataOverride, TResultOverride>['run']>
  ) => ReturnType<ExportedWorkflow<TData, TResult, TDataOverride, TResultOverride>['run']>
  /**
 ...
 */
export type UnwrapWorkflowInputDataType<T extends ReturnWorkflow<any, any, any>> =
