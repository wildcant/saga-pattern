...
import { isDefined, OrchestrationUtils } from '../../../utils'
...
export function when(...args) {
  ...
  return {
    then: fn => {
      ...
      const applyCondition = global[OrchestrationUtils.SymbolMedusaWorkflowComposerCondition].steps

      if (isDefined(ret) && ret?.__type !== OrchestrationUtils.SymbolWorkflowStep) {
        if (!isDefined(name)) {
          name = 'when-then-' + ulid()
          const context = global[OrchestrationUtils.SymbolMedusaWorkflowComposerContext]
        ...
        const retStep = createStep(name, ({ input }: { input: any }) => new StepResponse(input))
