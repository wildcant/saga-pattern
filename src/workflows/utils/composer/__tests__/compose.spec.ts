...
describe('Workflow composer', function () {
  ...
  describe('Using steps returning plain values', function () {
    ...
    it('should compose a workflow and pass down the event group id provided as part of the context', async () => {
      let context
      const mockStep1Fn = jest.fn().mockImplementation((input, { context: stepContext }) => {
        context = stepContext
      })
    ...
    it('should compose two new workflows concurrently and execute them concurrently', async () => {
      ...
      const [{ result: workflowResult }, { result: workflow2Result }] = await promiseAll([
        workflow().run({
          input: workflowInput,
        }),
        workflow2().run({
    ...
    it('should compose a new workflow and execute it multiple times concurrently', async () => {
      const mockStep1Fn = jest.fn().mockImplementation(function (input, context) {
        return { inputs: [input], obj: 'return from 1' }
      })
      ...
      const [{ result: workflowResult }, { result: workflowResult2 }] = await promiseAll([
        workflow().run({
          input: workflowInput,
        }),
        workflow().run({
    ...
    it('should transform the values before forward them to the next step', async () => {
      ...
      const transform1Fn = jest.fn().mockImplementation(({ input, step1Result }) => {
        const newObj = {
          ...step1Result,
          ...input,
          sum: input.a + input.b,
      ...
      const transform2Fn = jest.fn().mockImplementation(async ({ input }, context) => {
    ...
    it('should compose a new workflow and access properties from steps', async () => {
      ...
      const workflow = createWorkflow('workflow1', function (input) {
        ...
        parallelize(saveProduct({ product: data.product }), saveVariant({ variant: data.variant }))
      })
  ...
  describe('Using steps returning StepResponse', function () {
    ...
    it('should compose a new workflow and skip steps depending on the input', async () => {
      ...
      const workflow = createWorkflow('workflow1', function (input) {
        const returnStep1 = step1(input)
        const ret2 = step2(returnStep1)
        return new WorkflowResponse(step3({ one: returnStep1, two: ret2, input }))
      })
  ...
  it('should fail step and return response to compensate partial data', async () => {
    ...
    const mockStep1Fn = jest.fn().mockImplementation(async (input, context) => {
      ...
      if (errors.length > 0) {
        return StepResponse.permanentFailure('Error inserting ' + errors.join(', '), ok)
      }

      return new StepResponse(ok)
    })

    const mockStep1CompensateFn = jest.fn().mockImplementation((input, context) => {
      return input
    })

    const step1 = createStep({ name: 'step1', maxRetries }, mockStep1Fn, mockStep1CompensateFn)
