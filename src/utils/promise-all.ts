...
const getMessageError = (state: PromiseRejectedResult) => state.reason.message ?? state.reason

const isRejected = (state: PromiseSettledResult<unknown>): state is PromiseRejectedResult => {
...
export async function promiseAll<T extends readonly unknown[] | []>(
  ...
  const rejected = (states as PromiseSettledResult<unknown>[]).filter(isRejected)
  ...
  return (states as PromiseFulfilledResult<unknown>[]).map(getValue) as unknown as Promise<{
